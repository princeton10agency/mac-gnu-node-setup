...
_nvm_auto_use() {
  ...
  if [ -n "$nvmrc" ]; then
    ...
    if [ "$resolved" != "$current" ]; then
      nvm use "$wanted" >/dev/null && echo "nvm use $(nvm current)  # from ${nvmrc#$toplevel/}"
    fi
  fi
}
case "$PROMPT_COMMAND" in
  *"_nvm_auto_use"* ) : ;;
  "" ) PROMPT_COMMAND="_nvm_auto_use" ;;
  * )  PROMPT_COMMAND="_nvm_auto_use; $PROMPT_COMMAND" ;;
esac
_nvm_auto_use
# --- end nvm auto-use (bash) ---
